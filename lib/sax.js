...
sax.EVENTS = [ // for discoverability.
  ...
  "bindnamespace", "unbindnamespace", "attributens", 
  "error", "end", "ready" ]
...
function attrib (parser) { // xmlns
  if (parser.opt.xmlns) {
    ...
    if (n == 'xmlns' || qn.prefix == 'xmlns') {
      ...
      parser.tag.bindings.unshift(prefix)
    }
...
function openTag (parser, selfClosing) {
  if (parser.opt.xmlns) {
    ...
    for (var i = 0; i < parser.tag.bindings.length; i ++) {
      ...
      emitNode(parser, "onbindnamespace", { 
          prefix: p, namespace: parser.ns[p][0] })
    }
    ...
    if (parser.opt.xmlns) {
        ...
        while (parser.tag.bindings.length) {
            var p = parser.tag.bindings.pop()
              , n = parser.ns[p].pop()
            emitNode(parser, "onunbindnamespace", { prefix: p, namespace: n })
        }
    }
  }
